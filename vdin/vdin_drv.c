...
/*
 * reverse_flag:
 * bit0-3: bit0:vdin0 reverse enable/disable; bit1-2:reserved for vdin0 hvreverse
 * bit4-7: bit4:vdin1 reverse enable/disable; bit5-6:reserved for vdin1 hvreverse
 */
#if MESON_CPU_TYPE >= MESON_CPU_TYPE_MESON6TV
static int reverse_flag = 0;
module_param(reverse_flag, int,0644);
MODULE_PARM_DESC(reverse_flag,"reverse/disreverse vdin buffer & ");
#endif
...
static DEVICE_ATTR(sig_det,0666,sig_det_show,sig_det_store);
...
static void vdin_start_dec(struct vdin_dev_s *devp)
{
	...
	vdin_wr_reverse(devp, reverse_flag);
#endif
...
{
	int type = simple_strtol(buf, NULL, 0);
	memp_set(type);
	return count;
}
...
static int vdin_drv_probe(struct platform_device *pdev)
{
	...
	struct vdin_dev_s *vdevp;
	struct resource *res;
	int *pData = (int *)pdev->dev.platform_data;
    ...
    if(pData != NULL) {
        vdevp->mem_start = res->start - pData[0];
    } else {
