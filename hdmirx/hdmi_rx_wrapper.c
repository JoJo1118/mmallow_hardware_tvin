...
static unsigned sig_lost_lock_max = 30;
...
/* basic format*/
	...
	{HDMI_480i60,      0,  27000, 1440,  240,  240, 240	,2},//, 60},
	{HDMI_480i60_16x9, 0,  27000, 1440,  240,  240, 240	,2},//, 60},
	{HDMI_576p50,      0,  27000,  720,  576, 1201, 576	,0},//, 50},
	{HDMI_576p50_16x9, 0,  27000,  720,  576, 1201, 576	,0},//, 50},
	{HDMI_576i50,      0,  27000, 1440,  288,  288, 288	,2},//, 50},
	{HDMI_576i50_16x9, 0,  27000, 1440,  288,  288, 288	,2},//, 50},
	{HDMI_576i50_16x9, 0,  27000, 1440,  145,  145, 145	,10},//, 50},
...
/* extend format */
	{HDMI_1440x240p60,      0, 27000, 1440, 240, 240, 240	,2},//, 60},	//vic 8
	{HDMI_1440x240p60_16x9, 0, 27000, 1440, 240, 240, 240	,2},//, 60},	//vic 9
	{HDMI_2880x480i60,      0, 54000, 2880, 240, 240, 240	,10},//, 60},	//vic 10
	{HDMI_2880x480i60_16x9, 0, 54000, 2880, 240, 240, 240	,10},//, 60},	//vic 11
	{HDMI_2880x240p60,      0, 54000, 2880, 240, 240, 240	,10},//, 60},	//vic 12
	{HDMI_2880x240p60_16x9, 0, 54000, 2880, 240, 240, 240	,10},//, 60},	//vic 13
	{HDMI_1440x480p60,      0, 54000, 1440, 480, 480, 480	,10},//, 60},	//vic 14	repeat 1-2
	{HDMI_1440x480p60_16x9, 0, 54000, 1440, 480, 480, 480	,10},//, 60},	//vic 15	repeat 1-2

	{HDMI_1440x288p50,      0, 27000, 1440, 288, 288, 288	,2},//, 50},	//vic 23
	{HDMI_1440x288p50_16x9, 0, 27000, 1440, 288, 288, 288	,2},//, 50},	//vic 24
	{HDMI_2880x576i50,      0, 54000, 2880, 288, 288, 288	,10},//, 50},	//vic 25
	{HDMI_2880x576i50_16x9, 0, 54000, 2880, 288, 288, 288	,10},//, 50},	//vic 26
	{HDMI_2880x288p50,      0, 54000, 2880, 288, 288, 288	,10},//, 50},	//vic 27
	{HDMI_2880x288p50_16x9, 0, 54000, 2880, 288, 288, 288	,10},//, 50},	//vic 28
	{HDMI_1440x576p50,      0, 54000, 1440, 576, 576, 576	,10},//, 50},	//vic 29	repeat 1-2
	{HDMI_1440x576p50_16x9, 0, 54000, 1440, 576, 576, 576	,10},//, 50},	//vic 30	repeat 1-2

	{HDMI_2880x480p60,      0, 108000, 2880, 480,  480, 480	,10},//, 60},	//vic 35	repeat 1\2\4
	{HDMI_2880x480p60_16x9, 0, 108000, 2880, 480,  480, 480	,10},//, 60},	//vic 36	repeat 1\2\4
	{HDMI_2880x576p50,      0, 108000, 2880, 576,  576, 576	,10},//, 50},	//vic 37	repeat 1\2\4
	{HDMI_2880x576p50_16x9, 0, 108000, 2880, 576,  576, 576	,10},//, 50},	//vic 38	repeat 1\2\4
...
static int get_timing_fmt(struct hdmi_rx_ctrl_video *video_par)
{
    ...
    if (freq_ref[i].vic != 0) {
        ...
        if(repeat_check) {
			...
			if((freq_ref[i].repetition_times == 2)&&(video_par->pixel_repetition != 2)) {
				if(debug_mode & debug_repeat_bit)
  ...
  if(sm_pause)
	...
	case HDMIRX_HWSTATE_SIG_UNSTABLE:
		if(!tx_5v_status && !sw_5v_sts){
		    ...
		    if (hdmirx_rd_dwc(RA_HDMI_PLL_LCK_STS) & 0x01) {
		    	...
		    	if((sig_unlock_reset_cnt++ > sig_unlock_reset_max)&&(reset_sw)){
					sig_unlock_reset_cnt = 0;
	            	hdmirx_print("[HDMIRX State] PLL unlock: HDMIRX reset!\n");
					if(sig_unstable_reset_hpd_flag) {
						hdmirx_set_hpd(rx.port, 0);
						hdmirx_print("\n\n PLL lock but timing unstable----stable");
						rx.state = HDMIRX_HWSTATE_INIT;
					} else
						phy_init(rx.port, 0);
					sig_unstable_reset_hpd_flag = !sig_unstable_reset_hpd_flag;
		        }
	...
	case HDMIRX_HWSTATE_SIG_STABLE:
		if(!tx_5v_status && !sw_5v_sts){
		    ...
		    if ((reset_sw)&&(sig_unstable_reset_cnt++ > sig_unstable_reset_max)){
		        sig_unstable_reset_cnt = 0;
				if(sig_unstable_reset_hpd_flag) {
					hdmirx_set_hpd(rx.port, 0);
					hdmirx_print("\n\n PLL lock but timing unstable----stable");
