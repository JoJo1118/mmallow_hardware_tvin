...
inline void vdin_set_hvscale(struct vdin_dev_s *devp)
{
		...
		if(devp->parm.port>=TVIN_PORT_HDMI0 && devp->parm.port<=TVIN_PORT_HDMI7) {
			if((devp->parm.info.fmt == TVIN_SIG_FMT_HDMI_4096_2160_00HZ)||
				(devp->parm.info.fmt == TVIN_SIG_FMT_HDMI_4096_2160_00HZ)) {
				devp->scaler4h = 1080;
			}
		}
		*/
		...
		((devp->v_active > VPP_MAX_VACTIVE) && (!devp->prop.trans_fmt))) {
		if((devp->v_active > VPP_MAX_VACTIVE)&&(!devp->prop.trans_fmt)) {
			vdin_set_vscale(offset, devp->v_active, VPP_MAX_VACTIVE);
			devp->v_active = VPP_MAX_VACTIVE;
		} else {
...
#if MESON_CPU_TYPE >= MESON_CPU_TYPE_MESON6TV
inline void vdin_wr_reverse(struct vdin_dev_s *devp, unsigned int reverse)
{
    unsigned int offset = devp->addr_offset;

    if (devp->index == 0)
        devp->reverse_ctl = reverse & 0x0f;
    else
        devp->reverse_ctl = (reverse & 0xf0) >> 4;

    //pr_info("[vdin.%d] reverse ctrl:%x\n",devp->index, devp->reverse_ctl);

    if ((devp->reverse_ctl & 0x1) != 0) {
