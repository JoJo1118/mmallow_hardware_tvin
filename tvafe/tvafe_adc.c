...
typedef enum  bar_det_status_e{
	BAR_INIT_PRE,
	BAR_INIT,
	BAR_RDLEFT_FIRST,
	BAR_RDLEFT_SECOND,
	BAR_RDLEFT_THIRD,
	BAR_RDLEFT_SUCESSFUL,
	BAR_RDLEFT_NEXT,
	BAR_RDLEFT_FAILED,
}bar_det_status_t;
static bar_det_status_t bar_status;
...
static unsigned int bar_det_time = 4;
module_param(bar_det_time,int,0644);
...
int tvin_vdin_get_left_value(unsigned int *left1,unsigned int *left2,
	bar_det_status_t bar_status_cur)
{
	static unsigned skip_vs_count = 0;
	if((++skip_vs_count) < 2){
		bar_status=bar_status_cur;
		return 0;
	}
	skip_vs_count = 0;
	*left1 = READ_CBUS_REG_BITS(VDIN_BLKBAR_IND_LEFT1_CNT,BLKBAR_LEFT1_CNT_BIT,BLKBAR_LEFT1_CNT_WID);
	*left2 = READ_CBUS_REG_BITS(VDIN_BLKBAR_IND_LEFT2_CNT,BLKBAR_LEFT2_CNT_BIT,BLKBAR_LEFT2_CNT_WID);
	if(adc_dbg_en)
		pr_info("[BAR_INIT_PRE]%s: left1=[%d],left2=[%d].\n",__func__,*left1,*left2);
	return 1;
}

void tvin_vdin_bar_detect(enum tvin_sig_fmt_e fmt, struct tvafe_adc_s *adc)
{
        unsigned int hleft1,hleft2,hright1,hright2;
	...
	switch(bar_status)
	{
		case BAR_INIT_PRE:
			time_limit = jiffies;
			if(tvin_vdin_get_left_value(&hleft1,&hleft2,BAR_INIT_PRE) == 0)
				break;
			if(abs(hleft1 - fmt_info_p->v_active) > bar_det_delta || abs(hleft2 - fmt_info_p->v_active) > bar_det_delta){
				adc->vga_parm.hpos_step -= bar_width;
				adc->vga_auto.phase_state = VGA_BORDER_DET;
				adc->vga_auto.vs_cnt = 0;
				if((++bar_count)>bar_det_count){
					bar_status=BAR_RDLEFT_FAILED;
				}
				break;
			}
			bar_status = BAR_INIT;
		...
		case BAR_INIT:
			...
			if(adc_dbg_en)
				pr_info("[BAR_INIT]%s: barhstart=[%d],barhend=[%d].\n",__func__,barhstart,barhend);
			bar_status = BAR_RDLEFT_FIRST;
			break;
		case BAR_RDLEFT_FIRST:
			if(tvin_vdin_get_left_value(&hleft1,&hleft2,BAR_RDLEFT_FIRST) == 0)
				break;
			if(adc_dbg_en)
				pr_info("[first read]%s: hleft1=[%d],hleft2=[%d].\n",__func__,hleft1,hleft2);
			if(hleft1 >= (fmt_info_p->v_active>>black_bar_v1) && (hleft2 <= (fmt_info_p->v_active-(fmt_info_p->v_active>>black_bar_v2)))){
				bar_status=BAR_RDLEFT_SUCESSFUL;
				adc->vga_parm.hpos_step += (barhstart+1);
				if(adc_dbg_en)
					pr_info("[tvafe_adc..]%s: detect left border first successful,barstart=%u.\n",__func__,barhstart);
			}else{
				bar_status = BAR_RDLEFT_SECOND;
				if(adc_dbg_en)
					pr_info("[tvafe_adc..]%s: detect left border second faild,barstart=%u.\n",__func__,barhstart);

			}
			break;
		case BAR_RDLEFT_SECOND:
			if(tvin_vdin_get_left_value(&hleft1,&hleft2,BAR_RDLEFT_SECOND) == 0)
				break;
			if(adc_dbg_en)
				pr_info("[second read]%s: hleft1=[%d],hleft2=[%d].\n",__func__,hleft1,hleft2);
			if(hleft1 >= (fmt_info_p->v_active>>black_bar_v1) && (hleft2 <= (fmt_info_p->v_active-(fmt_info_p->v_active>>black_bar_v2)))){
				bar_status=BAR_RDLEFT_SUCESSFUL;
				adc->vga_parm.hpos_step += (barhstart+1);
				if(adc_dbg_en)
					pr_info("[tvafe_adc..]%s: detect left border second successful,barstart=%u.\n",__func__,barhstart);
			}else{
				bar_status = BAR_RDLEFT_THIRD;
				if(adc_dbg_en)
					pr_info("[tvafe_adc..]%s: detect left border second faild,barstart=%u.\n",__func__,barhstart);

			}
			break;
		case BAR_RDLEFT_THIRD:
			if(tvin_vdin_get_left_value(&hleft1,&hleft2,BAR_RDLEFT_THIRD) == 0)
				break;
			if(adc_dbg_en)
				pr_info("[third read]%s: hleft1=[%d],hleft2=[%d].\n",__func__,hleft1,hleft2);

			if(hleft1 >= (fmt_info_p->v_active>>black_bar_v1) && (hleft2 <= (fmt_info_p->v_active-(fmt_info_p->v_active>>black_bar_v2)))){
				bar_status=BAR_RDLEFT_SUCESSFUL;
				adc->vga_parm.hpos_step += (barhstart+1);
				if(adc_dbg_en)
					pr_info("[tvafe_adc..]%s: detect left border third  successful,barstart=%u.\n",__func__,barhstart);

			}else{
				bar_status = BAR_RDLEFT_NEXT;
				if(adc_dbg_en)
					pr_info("[tvafe_adc..]%s: detect left border third faild,barstart=%u.\n",__func__,barhstart);

			}
			break;
		case BAR_RDLEFT_NEXT:
			barhstart++;
			barhend++;
			if((barhstart > tvin_vga_fmt_tbl[fmt-1].hs_bp)||(jiffies-time_limit)/HZ > bar_det_time){
				bar_status=BAR_RDLEFT_FAILED;
				break;
			}
			...
			bar_status=BAR_RDLEFT_FIRST;
			break;
		case BAR_RDLEFT_FAILED:
                	pr_err("[vdin..]%s: vdin border detection failed.bar_count=[%d]\n",__func__,bar_count);
			adc->cmd_status = TVAFE_CMD_STATUS_SUCCESSFUL;
			adc->vga_auto.phase_state = VGA_PHASE_IDLE;
			adc->vga_parm.hpos_step+=bar_width*(bar_count+1);
			bar_count=0;
			break;
		case BAR_RDLEFT_SUCESSFUL:
			bar_count=0;
			adc->cmd_status = TVAFE_CMD_STATUS_SUCCESSFUL;
			adc->vga_auto.phase_state = VGA_PHASE_IDLE;
			break;
...
void tvin_vdin_bbar_init(enum tvin_sig_fmt_e fmt)
{
	...
	WRITE_CBUS_REG_BITS(VDIN_BLKBAR_CTRL0,3, BLKBAR_DIN_SEL_BIT, BLKBAR_DIN_SEL_WID);
	...
	switch (vga_auto->phase_state) {
		...
		case VGA_PHASE_END: //auto position
			...
			if (vga_auto->vs_cnt > AUTO_CLK_VS_CNT)
			{
				...
				if(((vga_auto->border.hend - vga_auto->border.hstart + 1) > tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_active) &&
					(vga_auto->border.hend >= (tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_front+tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_width + tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_bp + tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_active - 10))
					)
				{
				...
				else if (vga_auto->border.hstart < (tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_width + tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_bp))
				{
